...
\abstract{
This package helps dealing with the notation for linear elastic/piezoelectric materials by defining commands for fields and coefficients as well as support for easy switching between different notations. There is also support for handling the superscripts denoting the conditions for the coeffients. 
}
...
we can write \verb|\renewcommand{\stiffness]{C}|. This is how the standard notions are defined. As an example, the definition of \verb|\IkedaNotation| is
\begin{verbatim}
...
In this section, we rely on Mason's description in \cite{Mason1958} and stick to his notation. The constitutive equations can be derived from the thermodynamic potentials in table \ref{tab:thermodyn_potentials}.
...
\begin{align}
  ...
  \entropydensity & = -\frac{\partial U}{\partial\temperature}
                    = -\frac{\partial H}{\partial\temperature}
                    = -\frac{\partial H_1}{\partial\temperature}
                    = -\frac{\partial H_2}{\partial\temperature}.
\end{align}
