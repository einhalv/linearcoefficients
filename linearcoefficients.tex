...
{\NyeNotation
\begin{align}
  \strain_{ij} & = \compliance_{ijkl}\stress_{kl} + \piezod_{kij}\efield_k + \thermalexpansion_{ij}\Delta\temperature \label{eq:constitutive1G}\\
  \dfield_i & = \piezod_{ikl}\stress_{kl} + \permittivity_{ik}\efield_k + \pyroelec_i\Delta\temperature \label{eq:constitutive2G}\\
  \Delta\entropydensity & = \thermalexpansion_{kl}\stress_{kl} + \pyroelec_k\efield_k + (\heatcapacity/\temperature)\Delta\temperature.\label{eq:constitutive3G}
\end{align}
}

Sometimes additional superscripts to those automatically generated are desired. This can be accomplished by an optional argument to the coefficient-generating commands. For for example,  we have (Ikeda-notation):

{\IkedaNotation
\begin{tabular}{lcl}
  \rule{0pt}{3ex}\verb|\Gform\compliance[\,\prime\,]_{ijkl}| & $\Rightarrow$ & $\Gform\compliance[\,\prime\,]_{ijkl} $, \\
  \rule{0pt}{3ex}\verb|\Gform\piezod[\,\prime\,]_{kij}| & $\Rightarrow$ & $\Gform\piezod[\,\prime\,]_{kij}$, \\
  \rule{0pt}{3ex}\verb|\Gform\thermalexpansion[\,\prime\,]_{ij}| & $\Rightarrow$ & $\Gform\thermalexpansion[\,\prime\,]_{ij}$, \\
  \rule{0pt}{3ex}\verb|\Gform\permittivity[\,\prime\,]_{ik}| & $\Rightarrow$ & $\Gform\permittivity[\,\prime\,]_{ik}$, \\
  \rule{0pt}{3ex}\verb|\Gform\pyroelec[\,\prime\,]_{k}| & $\Rightarrow$ & $\Gform\pyroelec[\,\prime\,]_{k}$, \\
  \rule{0pt}{3ex}\verb|\Gform\heatcapacity[\,\prime\,]| & $\Rightarrow$ & $\Gform\heatcapacity[\,\prime\,]$.
\end{tabular}
}

\section{Matrix notation}
There is support for matrix notation for the tensors. Three different notations are defined: i) The macro \verb|\underlinemat| defines a notation where rectangular matrices are indicated by double underscores and column matrices are indicated by single underscores. ii) Brackets for rectangular matrices and braces for column matrices is set with the macro \verb|\bracketmat|. iii) Bold symbols for both rectangular and column matrices is by \verb|\boldfacemat|.

 The notations for respectively rectangular and column matrices are produced by the macros \verb|\mat| and \verb|\colmat|. There are also varieties of these called \verb|\matp| and \verb|\colmatp| that put a prime on the symbol in questions. For example
\begin{verbatim}
...
Replacing, \verb|\underlinemat| by \verb|\bracketmat| produces instead
{\IkedaNotation\bracketmat\nofree
\begin{align*}
  \colmat{\dfield} = \mat{\permittivity}\colmat{\efield},
  && \colmatp{\dfield} = \mat{a}\colmat{\dfield},
  && \colmatp{\efield} = \mat{a}\colmat{\efield},   
...
while \verb|\boldfacemat| produces
{\IkedaNotation\boldfacemat\nofree
\begin{align*}
  \colmat{\dfield} = \mat{\permittivity}\colmat{\efield},
  && \colmatp{\dfield} = \mat{a}\colmat{\dfield},
  && \colmatp{\efield} = \mat{a}\colmat{\efield},   
...
Either of the three matrix notations above can be specified by the respective package options \verb|underlinematrices|, \verb|bracketmatrices| or \verb|boldmatrices|. If more than one of these options are given, the last one is used. If no option is given, the package defaults to \verb|bracketmatrices|. 


The command \verb|\amat| automatically generates correct matrix representation of the coefficient or field quantity while treating superscripts correctly. Examples in Ikeda's notation are
{\IkedaNotation
\begin{align}
  \amat\polarization, && && \amat\efield, && \amat\dfield,
  &&  \amat\stress, && \amat\strain,
  && \amat\temperature, &&  \amat\entropydensity,\nonumber \\
  {\nofree\amat\stiffness}, && {\GEform\amat\stiffness},
  && {\nofree\amat\compliance}, && {\Gform\amat\compliance},
  && {\nofree\amat\permittivity}, && {\GEform\amat\permittivity},  
  && {\nofree\amat\impermittivity}, && {\GEform\amat\impermittivity},
  \nonumber \\
  {\nofree\amat\piezoh}, && {\GEform\amat\piezoh},
  && {\nofree\amat\piezog}, && {\GEform\amat\piezog},
  && {\nofree\amat\piezoe}, && {\GEform\amat\piezoe},
  && {\nofree\amat\piezod}, && {\GEform\amat\piezod}, \nonumber \\
  {\nofree\amat\heatcapacity}, && {\GEform\amat\heatcapacity},
  && {\nofree\amat\pyroelec}, && {\GEform\amat\pyroelec},
  && {\nofree\amat\heatofpolarization}, && {\GEform\amat\heatofpolarization},
  && {\nofree\amat\thermalexpansion}, && {\GEform\amat\thermalexpansion},
  && {\nofree\amat\heatofdeformation}, && {\GEform\amat\heatofdeformation}.\quad\nonumber
\end{align}
}
...
\begin{verbatim}
  {\nofree\amat\heatcapacity}, && {\GEform\amat\heatcapacity},
  && {\nofree\amat\pyroelec}, && {\GEform\amat\pyroelec},
  && {\nofree\amat\heatofpolarization},
  && {\GEform\amat\heatofpolarization},
  && {\nofree\amat\thermalexpansion},
  && {\GEform\amat\thermalexpansion},
  && {\nofree\amat\heatofdeformation},
  && {\GEform\amat\heatofdeformation}.
\end{verbatim}

As further examples, the constitutive equations (\ref{eq:constitutive1G})-(\ref{eq:constitutive3G}) can be given in the matrix notations as
{\NyeNotation\Gform\underlinemat
\begin{align*}
  \amat\strain & = \amat\compliance\amat\stress + {\amat\piezod}^\mathrm{T}\amat\efield + \amat\thermalexpansion\Delta\temperature \\
 \amat\dfield & = \amat\piezod\amat\stress + \amat\permittivity\amat\efield + \amat\pyroelec\Delta\temperature \\
  \Delta\entropydensity & = {\amat\thermalexpansion}^\mathrm{T}\amat\stress + {\amat\pyroelec}^\mathrm{T}\amat\efield + (\heatcapacity/\temperature)\Delta\temperature,
\end{align*}
}
{\NyeNotation\Gform\bracketmat
\begin{align*}
  \amat\strain & = \amat\compliance\amat\stress + {\amat\piezod}^\mathrm{T}\amat\efield + \amat\thermalexpansion\Delta\temperature \\
 \amat\dfield & = \amat\piezod\amat\stress + \amat\permittivity\amat\efield + \amat\pyroelec\Delta\temperature \\
  \Delta\entropydensity & = {\amat\thermalexpansion}^\mathrm{T}\amat\stress + {\amat\pyroelec}^\mathrm{T}\amat\efield + (\heatcapacity/\temperature)\Delta\temperature,
...
The options for inserting optionals superscripts also work in the matrix notations,
e.g.
{\IkedaNotation\Gform\underlinemat
  \begin{align*}
    \amat{\compliance[\,\prime\,]},
    && \amat{\piezod[\,\prime\,]},
    && \amat{\thermalexpansion[\,\prime\,]},
    && \amat{\permittivity[\,\prime\,]},
    && \amat{\pyroelec[\,\prime\,]},
    && \amat{\heatcapacity[\,\prime\,]},
  \end{align*}
}
{\IkedaNotation\Gform\bracketmat
  \begin{align*}
    \amat{\compliance[\,\prime\,]},
    && \amat{\piezod[\,\prime\,]},
    && \amat{\thermalexpansion[\,\prime\,]},
...
{\IkedaNotation\Gform\boldfacemat
  \begin{align*}
    \amat{\compliance[\,\prime\,]},
    && \amat{\piezod[\,\prime\,]},
    && \amat{\thermalexpansion[\,\prime\,]},
...
where each line was generated by the code
\begin{verbatim}
  \begin{align*}
    \amat{\compliance[\,\prime\,]},
    && \amat{\piezod[\,\prime\,]},
  ...
  \end{align*}
\end{verbatim}
Note that the argument to \verb|\amat| consists of multiple tokens and therefore must be enclosed in braces.
