...
TBD
\end{verbatim}
for a set of linear constitutive equations which renders as
\freestress\freeefield\freetemperature
\begin{align*}
  \strain_{ij} & = \compliance_{ijkl}\stress_{kl} + \piezod_{kij}\efield_k + (???)_{ij}\temperature \\
  \dfield_i & = \piezod_{ikl}\stress_{kl} + \permittivity_{ik}\efield_k + (???)_i\temperature \\
  \entropydensity & = (???)_{kl}\stress_{kl} + (???)_k\efield_k + (???)\temperature
\end{align*}
